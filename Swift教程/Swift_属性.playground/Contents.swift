...
//可以在定义存储属性的时候指定默认值
//也可以在构造过程中设置或修改存储属 性的值，甚至修改常量存储属性的值
...
//延迟存储属性 延迟存储属性是指当第一次被调用的时候才会计算其初始值的属性。在属性声明前使用 lazy 来标示一个延迟存 储属性。
...
//设置属性 center 的值会调用它的 setter 来修改属性 origin 的 x 和 y 的值，从而实现移动正方形到 新的位置。
...
//willSet观察器会将新的属性值作为常量参数传入，在willSet的实现代码中可以为这个参数指定一个名 称，如果不指定则参数仍然可用，这时使用默认名称newValue表示。
//同样, didSet观察器会将旧的属性值作为参数传入，可以为该参数命名或者使用默认参数名oldValue。如果 在didSet方法中再次对该属性赋值，那么新值会覆盖旧的值。

/*
 注意
 父类的属性在子类的构造器中被赋值时，它在父类中的 willSet和didSet观察器会被调用，随后才会调用子类的观察器。在父类初始化方法调用之前，子类给属性赋值时，观察器不会被调用
 */
...
class StepCounter {
    var totalSteps: Int = 0 {
        willSet(newTotalSteps) {
            print("About to see totalSteps to \(totalSteps)")
        }
        didSet {
...
//当totalSteps被设置新值的时候 它的willSet和didSet观察器都会被调用 即使新值和当前值w安全相同时也会被调用。
//例子中的willSet观察器将表示新值的参数自定义为newTotalSteps 这个观察器只是简单的将新的值输出

//didSet观察器在totalSteps的值改变后被调用 它把新值和旧值进行对比 如果步数增加了 就输出一个消息表示增加了多少步 didSet没有为旧值提供自定义名称 所以默认值oldValue表示旧值的参数名

/*
 注意
 如果将属性通过 in-out 方式传入函数，willSet和didSet也会调用。这是因为 in-out 参数采用了拷入 拷出模式:即在函数内部使用的是参数的 copy，函数结束后，又对参数重新赋值
 */
