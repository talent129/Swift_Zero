...
//现在已经创建了一个 Vehicle 的新实例，你可以访问它的 description 属性来打印车辆
...
//子类生成 子类生成指的是在一个已有类的基础上创建一个新的类。子类继承超类的特性，并且可以进一步完善。你还可以为子类添加新的特性。
...
//定义一个叫 Bicycle 的子类，继承成父类 Vehicle :
...
//除了它所继承的特性，Bicycle类还定义了一个默认值为false的存储型属性hasBasket(属性推断为 l)。

//默认情况下，你创建任何新的 Bicycle实例将不会有一个篮子(即hasBasket属性默认为false),创建该实例之后，你可以为特定的Bicycle实例设置hasBasket属性为true:
...
//重写属性
//可以重写继承来的实例属性或类型属性，提供自己定制的 getter 和 setter，或添加属性观察器使重写的属性 可以观察属性值什么时候发生改变。

//重写属性的 Getters 和 Setters
//可以提供定制的 getter(或 setter)来重写任意继承来的属性，无论继承来的属性是存储型的还是计算型的 属性。子类并不知道继承来的属性是存储型的还是计算型的，它只知道继承来的属性会有一个名字和类型。你在 重写一个属性时，必需将它的名字和类型都写出来。这样才能使编译器去检查你重写的属性是与超类中同名同类 型的属性相匹配的。
...
//当设置AutomaticCar的currentSpeed属性， 属性的didSet观察器jj就会自动的设置gear的属性，为新的速度选择一个合适的档位。具体来说就是 属性观察器将新的速度值除以10， 然后向下取得最接近的整数值，最后加1来得到gear的值。
...
//你可以通过把方法，属性或下标标记为final来防止它们被重写，只需要在声明关键字前加上final修饰符即 可(例如: final var， final func， final class func，以及 final subscript)。
